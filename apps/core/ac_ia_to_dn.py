...
处理流程（含 UTM 投影 + 30m 重采样）：
1. 根据震级计算搜索半径，结合可选的烈度 KML 范围确定输出区域（EPSG:4326）
2. 在 ac.tif（EPSG:4326）上裁剪搜索范围子集，并根据中心经度选取 UTM 带号
3. 将 ac 子集与 Ia.tif 均 Warp 到选定 UTM 坐标系并重采样到 30m 像元，
   确保二者在 UTM 下像素对齐（同范围、同分辨率、同尺寸）
4. 在 UTM 30m 网格上向量化计算 Dn，并写入输出文件（UTM 投影、30m 像元）

ac.tif 像元大小（参考）：0.0002780000000000000353 × -0.0002779999999999999811（EPSG:4326）
Ia.tif 坐标系与像元大小均动态获取（不强制要求 EPSG:4326）
"""
...
def get_utm_epsg(center_lon):
    """
    根据中心经度（北半球）返回对应 UTM 带的 EPSG 代码。

    支持 UTM 43-53 带（经度 72°–138°，北半球）。
    若中心经度超出该范围，记录 warning 并 clamp 到最近边界带号。

    参数:
        center_lon: 中心经度（度，WGS 84）

    返回:
        utm_epsg: UTM 带 EPSG 代码（整数，如 32648）
    """
    # (经度下界, 经度上界, EPSG)
    _utm_zones = [
        (72,  78,  32643),
    ...
    for lon_min, lon_max, epsg in _utm_zones:
        if lon_min <= center_lon < lon_max:
            return epsg
    # 超出支持范围：clamp 到最近边界带号
    if center_lon < 72:
        logger.warning(
            "中心经度 %.4f 超出支持的 UTM 范围（72°–138°），clamp 至 UTM43 (EPSG:32643)",
            center_lon
        )
        return 32643
    else:
        logger.warning(
            "中心经度 %.4f 超出支持的 UTM 范围（72°–138°），clamp 至 UTM53 (EPSG:32653)",
            center_lon
        )
        return 32653
...
def calculate_dn_optimized(ac_tif_path, ia_tif_path, output_path,
    ...
    处理流程：
        1. 根据震级计算搜索范围（可选与烈度 KML 合并）
        2. 将 ac.tif 子集与 Ia.tif 投影到 UTM 坐标系并重采样到 30m 像元
        3. 在 UTM 30m 网格上向量化计算 Dn，输出为 UTM 投影、30m 像元的 GeoTIFF

    参数:
        ac_tif_path: ac.tif文件路径（a_c 单位为 m/s²，EPSG:4326）
        ia_tif_path: Ia.tif文件路径（Ia 单位为 m/s，坐标系动态获取）
        output_path: 输出Dn.tif文件路径（Dn 单位为 cm，UTM 投影、30m 像元）
        epicenter_lon: 震中经度
        epicenter_lat: 震中纬度
        magnitude: 震级
...
def _calculate_dn_optimized_impl(ac_tif_path, ia_tif_path, output_path,
    ...
    try:
        ...
        # 确保像素坐标在有效范围内（ac.tif 边界裁剪，在 EPSG:4326 下完成）
        ...
        # ----------------------------------------------------------
        # 4. 打开 Ia.tif 并动态获取其坐标系（不强制 EPSG:4326）
        # ----------------------------------------------------------
        ia_dataset = gdal.Open(ia_tif_path, gdal.GA_ReadOnly)
        if ia_dataset is None:
            raise IOError(f"无法打开Ia.tif文件: {ia_tif_path}")

        ia_projection = ia_dataset.GetProjection()
        if not ia_projection:
            logger.warning("Ia.tif 没有投影信息，假定为 EPSG:4326: %s", ia_tif_path)
            ia_src_srs = 'EPSG:4326'
        else:
            logger.info("Ia.tif 投影信息已动态读取: %s", ia_tif_path)
            ia_src_srs = ia_projection
        ...
        # ----------------------------------------------------------
        # 步骤 A: 在 EPSG:4326 下裁剪 ac.tif 子集到内存
        # ----------------------------------------------------------
        # 从裁剪后的像素坐标推算实际地理范围
        actual_min_lon = ac_geotransform[0] + col_min * ac_geotransform[1]
        actual_max_lat = ac_geotransform[3] + row_min * ac_geotransform[5]
        actual_max_lon = ac_geotransform[0] + col_max * ac_geotransform[1]
        actual_min_lat = ac_geotransform[3] + row_max * ac_geotransform[5]

        logger.info(
            "步骤A: 裁剪 ac.tif 子集范围: lon=[%.6f, %.6f], lat=[%.6f, %.6f]",
            actual_min_lon, actual_max_lon, actual_min_lat, actual_max_lat
        )

        translate_opts = gdal.TranslateOptions(
            format='MEM',
            projWin=[actual_min_lon, actual_max_lat, actual_max_lon, actual_min_lat],
        )
        ac_subset_ds = gdal.Translate('', ac_dataset, options=translate_opts)
        if ac_subset_ds is None:
            raise IOError("裁剪 ac.tif 子集失败（gdal.Translate 返回 None）")

        logger.debug(
            "步骤A: ac_subset 尺寸: %dx%d",
            ac_subset_ds.RasterXSize, ac_subset_ds.RasterYSize
        )

        # ----------------------------------------------------------
        # 步骤 B: 根据中心经度选择 UTM 带号
        # ----------------------------------------------------------
        center_lon = (actual_min_lon + actual_max_lon) / 2
        utm_epsg = get_utm_epsg(center_lon)
        utm_zone_number = utm_epsg - 32600
        utm_srs = f"EPSG:{utm_epsg}"

        logger.info(
            "选定UTM带: %d (EPSG:%d), 中心经度=%.4f",
            utm_zone_number, utm_epsg, center_lon
        )

        # ----------------------------------------------------------
        # 步骤 C: 将 ac 子集与 Ia 都投影到 UTM 30m，并对齐到相同范围
        # ----------------------------------------------------------

        # C1: 将 ac 子集 Warp 到 UTM 30m
        warp_opts_ac = gdal.WarpOptions(
            format='MEM',
            dstSRS=utm_srs,
            xRes=30, yRes=30,
        ...
        ac_utm = gdal.Warp('', ac_subset_ds, options=warp_opts_ac)
        if ac_utm is None:
            raise IOError("将 ac 子集 Warp 到 UTM 失败（gdal.Warp 返回 None）")

        ac_utm_gt = ac_utm.GetGeoTransform()
        ac_utm_proj = ac_utm.GetProjection()
        logger.info(
            "步骤C: ac_utm 尺寸: %dx%d, GeoTransform: %s, 像元大小: %.2f x %.2f m",
            ac_utm.RasterXSize, ac_utm.RasterYSize,
            ac_utm_gt, ac_utm_gt[1], abs(ac_utm_gt[5])
        )

        # C2: 从 ac_utm 的 GeoTransform 推算 UTM 边界范围
        utm_x_min = ac_utm_gt[0]
        utm_y_max = ac_utm_gt[3]
        utm_x_max = ac_utm_gt[0] + ac_utm.RasterXSize * ac_utm_gt[1]
        utm_y_min = ac_utm_gt[3] + ac_utm.RasterYSize * ac_utm_gt[5]

        # C3: 将 Ia.tif Warp 到 UTM 30m，与 ac_utm 完全对齐（同范围、同分辨率、同尺寸）
        # 若 Ia 没有投影信息，则通过 srcSRS 指定为 EPSG:4326
        # ia_warp_dst_nodata: 若 Ia 无 nodata 元数据，使用 NODATA_VALUE 作为 Warp 填充值
        ia_warp_dst_nodata = ia_nodata if ia_nodata is not None else NODATA_VALUE
        warp_opts_ia = gdal.WarpOptions(
            format='MEM',
            srcSRS=ia_src_srs,
            dstSRS=utm_srs,
            xRes=30, yRes=30,
            resampleAlg=gdal.GRA_Bilinear,
            outputBounds=[utm_x_min, utm_y_min, utm_x_max, utm_y_max],
            srcNodata=ia_nodata,
            dstNodata=ia_warp_dst_nodata,
        )
        ia_utm = gdal.Warp('', ia_dataset, options=warp_opts_ia)
        if ia_utm is None:
            raise IOError("将 Ia.tif Warp 到 UTM 失败（gdal.Warp 返回 None）")

        ia_utm_gt = ia_utm.GetGeoTransform()
        logger.info(
            "步骤C: ia_utm 尺寸: %dx%d, GeoTransform: %s, 像元大小: %.2f x %.2f m",
            ia_utm.RasterXSize, ia_utm.RasterYSize,
            ia_utm_gt, ia_utm_gt[1], abs(ia_utm_gt[5])
        )

        # ----------------------------------------------------------
        # 步骤 D: 在 UTM 30m 网格上向量化计算 Dn
        # ----------------------------------------------------------
        utm_width = ac_utm.RasterXSize
        utm_height = ac_utm.RasterYSize

        # 读取 ac_utm 数据
        ac_utm_band = ac_utm.GetRasterBand(1)
        ac_utm_nodata = ac_utm_band.GetNoDataValue()
        if ac_utm_nodata is None:
            ac_utm_nodata = ac_nodata  # fallback 到原始 nodata

        ac_data_raw = ac_utm_band.ReadAsArray(0, 0, utm_width, utm_height)
        if ac_data_raw is None:
            raise IOError("读取 ac_utm 数据失败")
        ac_data = ac_data_raw.astype(np.float64)

        # 构建 ac nodata 掩码 (True = 该像素是 nodata)
        ac_is_nodata = _nodata_mask(ac_data, ac_utm_nodata)
        ...
        # 读取 ia_utm 数据（与 ac_utm 像素一一对应，无需坐标网格映射）
        ia_utm_band = ia_utm.GetRasterBand(1)
        ia_data_raw = ia_utm_band.ReadAsArray(0, 0, utm_width, utm_height)
        if ia_data_raw is None:
            raise IOError("读取 ia_utm 数据失败")
        ...
        ia_is_nodata = _nodata_mask(ia_data, ia_warp_dst_nodata)
        # Ia 有效 = ac 有值 & 不是 nodata & 值 > 0
        ia_is_usable = ac_is_valid & (~ia_is_nodata) & (ia_data > 0)
        ...
        if ia_usable_count > 0:
            logger.info(
                ...
                float(np.min(ia_data[ia_is_usable])),
                float(np.max(ia_data[ia_is_usable]))
            )
        else:
            logger.warning(
                "ac 有效区域与 Ia.tif 没有可用数据（Ia<=0 或全为 nodata），"
                "所有 ac 有值处 Dn 赋值为 0"
            )
        ...
        if np.any(calc_mask):
            with np.errstate(divide='ignore', invalid='ignore'):
                log_ia = np.log10(ia_data[calc_mask])
        ...
        # 使用 UTM 30m 的 GeoTransform 与 Projection
        out_dataset.SetGeoTransform(ac_utm_gt)
        out_dataset.SetProjection(ac_utm_proj)
        ...
        # ----------------------------------------------------------
        # 确保所有 GDAL 资源释放（包括 MEM 数据集）
        # ----------------------------------------------------------
        if out_dataset is not None:
            out_dataset = None
        if ia_utm is not None:
            ia_utm = None
        if ac_utm is not None:
            ac_utm = None
        if ac_subset_ds is not None:
            ac_subset_ds = None
