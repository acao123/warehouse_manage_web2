...
def _read_geology_grouping(tif_source):
    """
    从地质构造图层的属性表中读取 Grouping 字段的颜色和名称

    参数:
        tif_source: 地质构造图层的源路径

    返回:
        包含颜色 (R, G, B) 和 Grouping 名称的列表
    """
    result = []
    try:
        # 打开属性表
        layer = QgsVectorLayer(tif_source, "geology_layer", "ogr")
        # if not layer.isValid():
        #     raise Exception("无效的地质构造图层")

        # 获取字段索引
        # fields = layer.fields()
        # grouping_idx = fields.indexOf("Grouping")
        # if grouping_idx == -1:
        #     raise Exception("属性表中未找到 Grouping 字段")

        # 遍历要素，读取颜色和 Grouping 名称
        for feature in layer.getFeatures():
            grouping_value = feature["Grouping"]
            color = feature["color"]  # 假设颜色存储在 color 字段中
            r, g, b = color.red(), color.green(), color.blue()
            result.append((r, g, b, grouping_value))

    except Exception as e:
        print(f"读取 Grouping 字段失败: {e}")
...
def _add_legend(layout, map_item, lx, ly, lw, lh,
    ...
    if geology_layer is not None:
        tif_source = geology_layer.source()
        grouping_data = _read_geology_grouping(tif_source)

    if grouping_data:
        grouping_start_y = sep_y + 1.5  # 分隔线下方留1.5mm

        # Grouping 区域标题
        grp_title = QgsLayoutItemLabel(layout)
        grp_title.setText("Grouping")
        grp_title.setTextFormat(_make_text_format("宋体", 8, QColor(0, 0, 0)))
        grp_title.setHAlign(Qt.AlignCenter)
        grp_title.attemptResize(QgsLayoutSize(lw, 5.0, QgsUnitTypes.LayoutMillimeters))
        grp_title.attemptMove(QgsLayoutPoint(lx, grouping_start_y,
                                             QgsUnitTypes.LayoutMillimeters))
        grp_title.setBackgroundEnabled(False)
        grp_title.setFrameEnabled(False)
        layout.addLayoutItem(grp_title)

        # 每个 Grouping 项目：色块 + 文字
        item_h = 4.5       # 每项高度(mm)
        block_w = 7.0      # 色块宽度(mm)
        block_h = 3.5      # 色块高度(mm)
        text_x = lx + block_w + 2.5   # 文字起始X
        ...
        for i, (r, g, b, grouping_text) in enumerate(grouping_data):
            item_y = grouping_start_y + 5.5 + i * item_h
            if item_y + item_h > ly + lh - 1.0:
                # 超出图例面板，停止
                break
            ...
            # Grouping 文字
            text_lbl = QgsLayoutItemLabel(layout)
            text_lbl.setText(grouping_text)
            text_lbl.setTextFormat(_make_text_format("宋体", 7, QColor(0, 0, 0)))
            text_lbl.attemptResize(QgsLayoutSize(text_w, item_h,
                                                  QgsUnitTypes.LayoutMillimeters))
            text_lbl.attemptMove(QgsLayoutPoint(
