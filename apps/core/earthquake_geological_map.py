...
输出一个png图(图的总宽度为200mm)
	...
	4.指北针位置：放在地图的右上角，上边和地图上边框对齐，右侧和地图右边框对齐；样式参考”制图布局参考图3.png“，指北针样式：白色背景，黑色0.35mm边框，指针左侧是黑色半箭头，右侧是白色半箭头，箭头顶端写N；
    5.图例位置：放在地图的右侧，图例左边框与地图右边框重合，图例下边框与地图下边框平行，图例布局完全参考”制图布局参考图3.png“
	...
	8.地市级以上居民地res2_4m shp文件中，代表市的位置符号更改，具体样式为：黑色空圈内为一个实心黑圆，加一个圆形的白色背景；整体大小为市名称大小的三分之一；市的名称字体是9pt，颜色是黑色，加白边
	9.烈度的加载参考earthquake_kml_map.py，烈度圈展示在地图图层的最上边。
	...
	说明： 地质构造图tif文件位置：../../data/geology/图3/group.tif
	      ...
	      地市级以上居民地res2_4m shp文件位置：../../data/geology/地市级以上居民地res2_4m/res2_4m.shp
	12.把 地市级以上居民地res2_4m shp文件中属性表中PINYIN列内容改为汉字展示
	13.注释是中文注释，要求方法和参数需要有中文注释
	14.指省界颜色改为:R=160 G=160 B=160，0.4mm，市界颜色改为:R=160 G=160 B=160，0.24mm，虚线间隔为0.3，县界颜色改为:R=160 G=160 B=160，0.14mm虚线间隔为0.3
	15.代码需要无bug可运行，并写出测试方法
	输出图布局参考：制图布局参考图3.png
	输出完整代码，代码可能比较长，分四个部分输出，我会放到一个python文件中
	16.基于QGIS3.40.15 python环境，所以生产时一定需要参考对应版本API,生成的代码不能有bug
