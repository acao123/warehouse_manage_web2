...
参考 earthquake_geological_map.py 的布局、指北针、经纬度、比例尺、省市加载方式。
...
def _add_legend(layout, map_item, project, map_height_mm, output_height_mm, yanxing_list=None,
    ...
    row_count = (len(legend_items) + col_count - 1) // col_count
    ...
    if yanxing_list:
        ...
        yanxing_title_label.attemptMove(QgsLayoutPoint(legend_x, yanxing_title_y,
                                                       QgsUnitTypes.LayoutMillimeters))
        yanxing_title_label.attemptResize(QgsLayoutSize(legend_width, 4.5,
                                                        QgsUnitTypes.LayoutMillimeters))
        ...
        # 岩性图例项起始Y坐标（标题下方留0.5mm间距）
        item_start_y = yanxing_title_y + 4.5 + 0.5
        ...
        displayed_count = 0
        # 岩性图例底部裁剪边界：为比例尺预留空间
        yanxing_clip_y = legend_y + legend_height - scale_bar_reserved - 1.0
        ...
        for idx, (value, color_rgba, yanxing_name) in enumerate(yanxing_list):
            ...
            item_height = max(yanxing_icon_height, text_total_height) + 0.8

            if current_y + item_height > yanxing_clip_y:
                break
    ...
    if scale is not None and extent is not None and center_lat is not None:
        ...
        sb_y = legend_y + legend_height - sb_height - 2.0
        sb_x = legend_x + (legend_width - std_bar_width) / 2.0

        scale_tf = QgsTextFormat()
        scale_tf.setFont(QFont("Times New Roman", SCALE_FONT_SIZE_PT))
        ...
        bar_y = sb_y + 5.0 * scale_factor
        bar_h = 1.6 * scale_factor
        ...
        tick_tf.setFont(QFont("Times New Roman", SCALE_FONT_SIZE_PT))
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].lower() == "test":
        ...
        try:
            ...
            print(f"[错误] 参数格式错误: {e}")
            print("用法: python earthquake_geological_map2.py <经度> <纬度> <震级> [输出文件名]")
    else:
