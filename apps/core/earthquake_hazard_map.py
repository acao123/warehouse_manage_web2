...
def build_statistics_summary(area_stats):
    ...
    for level_name in HAZARD_LEVEL_NAMES:
        ...
        # 面积格式化：小于1保留两位小数，否则四舍五入取整
        if area_km2 < 1:
            area_str = f"{area_km2:.2f}"
        else:
            area_str = str(int(round(area_km2)))
...
def generate_earthquake_hazard_map(longitude, latitude, magnitude,
    ...
    try:
        return _generate_earthquake_hazard_map_impl(
            longitude, latitude, magnitude, a, b, c, output_path, dn_tif_path
        )
    except Exception as exc:
...
def run_all_tests():
    ...
    # Verify area formatting: 1500 -> "1500", small < 1 -> keep 2 decimal
    assert "1500" in summary, f"面积1500.0应格式化为1500，实际: {summary}"
    print(f"  统计摘要生成正确 ✓")
    ...
    if len(sys.argv) > 1 and sys.argv[1].lower() == "test":
        ...
        try:
            ...
            _img_path, _max_dn, _summary = generate_earthquake_hazard_map(
                _lon, _lat, _mag, _a, _b, _c, _out, _dn_tif)
            print(f"\n最大Dn值: {_max_dn} cm")
            print(f"\n统计摘要:\n{_summary}")
