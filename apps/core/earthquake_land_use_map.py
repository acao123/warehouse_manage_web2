# -*- coding: utf-8 -*-
"""
基于QGIS 3.40.15的地震震中土地利用类型图生成脚本
...
土地利用类型图例说明：
- 底图为土地利用类型TIF文件
- 图例按照ArcGIS标准土地利用分类显示
- Value值对应土地利用类型代码，如11=水田，12=旱地，21=有林地等
- 图例显示：色块 + 土地利用类型名称
...
土地利用类型分类标准：
    ...
    61: 沙地          62: 戈壁          63: 盐碱地        64: 沼泽地
    65: 裸土地        66: 裸岩石质地    67: 其他
    99: 海洋
"""
...
# 日志配置
# ============================================================
logger = logging.getLogger('report.core.earthquake_land_use_map')
...
# 土地利用类型TIF文件路径
LAND_USE_TIF_PATH = (
    getattr(_django_settings, 'LAND_USE_TIF_PATH',
            _DEFAULT_BASE + '图5/全国土壤利用分类.tif')
    if _DJANGO_AVAILABLE else _DEFAULT_BASE + '图5/全国土壤利用分类.tif'
)
...
# === 布局尺寸常量 ===
MAP_TOTAL_WIDTH_MM = 220.0  # 总宽度(mm)
LEGEND_WIDTH_MM = 50.0  # 图例宽度(mm)
BORDER_LEFT_MM = 4.0  # 左边距(mm)
BORDER_TOP_MM = 4.0  # 上边距(mm)
BORDER_BOTTOM_MM = 2.0  # 下边距(mm)
BORDER_RIGHT_MM = 1.0  # 右边距(mm)
MAP_WIDTH_MM = MAP_TOTAL_WIDTH_MM - BORDER_LEFT_MM - LEGEND_WIDTH_MM - BORDER_RIGHT_MM  # 地图宽度(mm)
...
LAND_USE_CLASSIFICATION = {
    ...
    , 32: ("中覆盖度草地", (115, 178, 255, 255))
    , 33: ("低覆盖度草地", (190, 210, 255, 255))
    , 41: ("河渠", (104, 104, 104, 255))
    , 42: ("湖泊", (130, 130, 130, 255))
    , 43: ("水库坑塘", (156, 156, 156, 255))
    , 44: ("永久性冰川雪地", (178, 178, 178, 255))
    , 45: ("滩涂", (204, 204, 204, 255))
    , 46: ("滩地", (225, 225, 225, 255))
    , 51: ("城镇用地", (230, 0, 169, 255))
    , 52: ("农村居民点", (255, 0, 197, 255))
    , 53: ("其它建设用地", (255, 115, 223, 255))
    , 61: ("沙地", (255, 235, 175, 255))
    , 62: ("戈壁", (255, 211, 127, 255))
    , 63: ("盐碱地", (255, 170, 0, 255))
    , 64: ("沼泽地", (230, 152, 0, 255))
    , 65: ("裸土地", (168, 112, 0, 255))
    , 66: ("裸岩石质地", (115, 76, 0, 255))
    , 67: ("其他", (215, 194, 158, 255))
...
def get_magnitude_config(magnitude):
    ...
    返回:
        dict - 包含radius_km, map_size_km, scale等配置项的字典
    """
    if magnitude < 6:
...
def get_land_use_name(value):
    ...
    返回:
        str - 土地利用类型名称
    """
    if value in LAND_USE_CLASSIFICATION:
        return LAND_USE_CLASSIFICATION[value][0]
    return f"类型{value}"


def get_land_use_color(value):
    """
    根据土地利用类型代码获取颜色
    ...
    """
    if value in LAND_USE_CLASSIFICATION:
        return LAND_USE_CLASSIFICATION[value][1]
...
def _parse_qml_colors_by_value(tif_path):
    ...
    返回:
        dict - {value(int): (r, g, b, 255)} 颜色映射字典
    """
    base = os.path.splitext(tif_path)[0]
    qml_candidates = [
        base + ".qml",
        tif_path + ".qml",
        os.path.join(os.path.dirname(tif_path), "全国土壤利用分类.qml"),
    ]
    qml_path = None
...
def build_land_use_legend_list(tif_path, raster_layer=None):
    """
    构建土地利用类型图例列表
    优先从QML或栅格图层获取颜色，如未找到则使用预定义的LAND_USE_CLASSIFICATION

    参数:
        tif_path: str - TIF文件路径
        raster_layer: QgsRasterLayer - 可选，栅格图层对象

    返回:
        list - [(value, (r,g,b,255), land_use_name), ...] 图例项列表
    """
    # 尝试从QML获取颜色
    ...
    # 按照预定义分类顺序构建图例
    # 这样确保图例按照标准土地利用类型顺序显示
    for value in sorted(LAND_USE_CLASSIFICATION.keys()):
        name = LAND_USE_CLASSIFICATION[value][0]
        ...
        if value in color_map:
            ...
            # 使用预定义颜色
            color = LAND_USE_CLASSIFICATION[value][1]

        result.append((value, color, name))

    print(f"  构建土地利用类型图例列表完成，共 {len(result)} 项")
    return result


def get_actual_land_use_values_from_raster(raster_layer):
    """
    从栅格图层获取实际存在的土地利用类型值
    用于只显示地图范围内实际存在的土地利用类型
...
def parse_intensity_kml(kml_path):
    ...
    返回:
        list - [{"intensity": int, "coords": [(lon, lat), ...]}, ...] 烈度圈数据列表
    """
...
def _extract_kml_linestring_coords(placemark, ns):
    ...
    参数:
        placemark: Element - KML Placemark元素
        ns: str - XML命名空间
...
def load_land_use_raster(tif_path):
    """
    加载土地利用类型TIF栅格图层

    参数:
        tif_path: str - TIF文件相对路径
    ...
    if not os.path.exists(abs_path):
        print(f"[错误] 土地利用类型底图文件不存在: {abs_path}")
        return None
    layer = QgsRasterLayer(abs_path, "土地利用类型底图")
    if not layer.isValid():
        print(f"[错误] 无法加载土地利用类型底图: {abs_path}")
        return None
    print(f"[信息] 成功加载土地利用类型底图: {abs_path}")
    return layer
...
def create_print_layout(project, longitude, latitude, magnitude, extent, scale, map_height_mm, land_use_list=None, ordered_layers=None):
    ...
    # 添加图例
    _add_legend(layout, map_item, project, map_height_mm, output_height_mm, land_use_list)
...
def _add_north_arrow(layout, map_height_mm):
    ...
    # 创建指北针SVG
    svg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_north_arrow_temp_landuse.svg")
...
def _add_legend(layout, map_item, project, map_height_mm, output_height_mm, land_use_list=None):
    """
    添加图例
    ...
    参数:
        ...
        map_height_mm: float - 地图高度(毫米)
        output_height_mm: float - 输出总高度(毫米)
        land_use_list: list - 土地利用类型图例列表
    """
    ...
    # 土地利用类型图例文本格式
    land_use_format = QgsTextFormat()
    land_use_format.setFont(QFont("SimSun", LEGEND_LAND_USE_FONT_SIZE_PT))
    land_use_format.setSize(LEGEND_LAND_USE_FONT_SIZE_PT)
    land_use_format.setSizeUnit(QgsUnitTypes.RenderPoints)
    land_use_format.setColor(QColor(0, 0, 0))
    ...
    if land_use_list:
        ...
        land_use_title_font = QFont("SimHei")
        land_use_title_font.setPointSizeF(10.0)
        land_use_title_format.setFont(land_use_title_font)
        land_use_title_format.setSize(10.0)
        land_use_title_format.setSizeUnit(QgsUnitTypes.RenderPoints)
        ...
        land_use_title_label.setHAlign(Qt.AlignHCenter)
        land_use_title_label.setVAlign(Qt.AlignVCenter)
        land_use_title_label.setFrameEnabled(False)
        land_use_title_label.setBackgroundEnabled(False)
        layout.addLayoutItem(land_use_title_label)

        item_start_y = land_use_title_y + 5.0

        land_use_icon_width = 5.0
        land_use_icon_height = 2.5
        land_use_gap = 1.0
        land_use_left_pad = 2.0
        land_use_right_pad = 2.0

        text_area_width = legend_width - land_use_left_pad - land_use_icon_width - land_use_gap - land_use_right_pad

        # 计算每行最大字符数
        char_width_mm = LEGEND_LAND_USE_FONT_SIZE_PT * 0.353 * 0.9
        max_chars_per_line = int(text_area_width / char_width_mm)
        if max_chars_per_line < 4:
            max_chars_per_line = 4

        line_height_mm = LEGEND_LAND_USE_FONT_SIZE_PT * 0.353 + 0.3

        current_y = item_start_y
        displayed_count = 0

        for idx, (value, color_rgba, land_use_name) in enumerate(land_use_list):
            # 计算文本行数和高度
            text_lines = _wrap_text(land_use_name, max_chars_per_line)
            num_lines = len(text_lines)

            text_total_height = num_lines * line_height_mm
            item_height = max(land_use_icon_height, text_total_height) + 1.0
            ...
            color_box.attemptMove(QgsLayoutPoint(legend_x + land_use_left_pad, icon_y,
                                                 QgsUnitTypes.LayoutMillimeters))
            color_box.attemptResize(QgsLayoutSize(land_use_icon_width, land_use_icon_height,
                                                  QgsUnitTypes.LayoutMillimeters))
            ...
            # 添加文字标签
            text_x = legend_x + land_use_left_pad + land_use_icon_width + land_use_gap
            wrapped_text = "\n".join(text_lines)

            text_label = QgsLayoutItemLabel(layout)
            text_label.setText(wrapped_text)
            text_label.setTextFormat(land_use_format)
        ...
        print(f"[信息] 土地利用类型图例添加完成，共 {displayed_count} 项，字体大小 {LEGEND_LAND_USE_FONT_SIZE_PT}pt")
    else:
        print("[信息] 无土地利用类型数据，跳过土地利用类型图例")
...
def generate_earthquake_land_use_map(longitude, latitude, magnitude,
    ...
    参数:
        ...
        kml_path: str - 可选，烈度圈KML文件路径

    返回:
        str - 输出文件的绝对路径，失败返回None
    """
    logger.info('开始生成土地利用类型图: lon=%.4f lat=%.4f M=%.1f output=%s',
...
def _generate_earthquake_land_use_map_impl(longitude, latitude, magnitude,
    ...
    try:
        ...
        # 加载土地利用类型底图
        land_use_layer = load_land_use_raster(LAND_USE_TIF_PATH)
        if land_use_layer:
            project.addMapLayer(land_use_layer)

        # 构建土地利用类型图例列表
        tif_abs_path = resolve_path(LAND_USE_TIF_PATH)
        land_use_list = build_land_use_legend_list(tif_abs_path, land_use_layer)
        print(f"[信息] 获取到 {len(land_use_list)} 个土地利用类型图例项")
        ...
        ordered_layers = [lyr for lyr in [
            epicenter_layer,        # 震中放在最上层
            annotation_raster,      # 天地图注记（放置在震中图层之下）
            intensity_layer,
            city_point_layer,
        ...
        # 清理指北针临时SVG文件
        svg_temp = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_north_arrow_temp_landuse.svg")
    ...
    print("=" * 60)
    if result:
        print(f"[完成] 土地利用类型图已输出: {result}")
    else:
        print("[失败] 土地利用类型图输出失败")
    print("=" * 60)
    return result
...
def test_land_use_classification():
    ...
    for value, expected_name in expected_types.items():
        assert value in LAND_USE_CLASSIFICATION, f"Value {value} 不在分类映射中"
        actual_name = LAND_USE_CLASSIFICATION[value][0]
        assert actual_name == expected_name, f"Value {value}: 期望'{expected_name}', 实际'{actual_name}'"

        # 验证颜色格式
        color = LAND_USE_CLASSIFICATION[value][1]
        assert len(color) == 4, f"Value {value}: 颜色应为4元组(R,G,B,A)"
        assert all(0 <= c <= 255 for c in color), f"Value {value}: 颜色值应在0-255范围内"

    print(f"  共验证 {len(expected_types)} 个土地利用类型分类 ✓")
    print("  土地利用类型分类映射测试通过 ✓")


def test_get_land_use_name():
    """测试获取土地利用类型名称"""
    print("\n--- 测试: get_land_use_name ---")

    assert get_land_use_name(11) == "水田"
    assert get_land_use_name(21) == "有林地"
    assert get_land_use_name(51) == "城镇用地"
    assert get_land_use_name(99) == "海洋"
    assert get_land_use_name(999) == "类型999"  # 未定义的类型

    print("  get_land_use_name(11) = '水田' ✓")
    print("  get_land_use_name(21) = '有林地' ✓")
    print("  get_land_use_name(51) = '城镇用地' ✓")
    print("  get_land_use_name(99) = '海洋' ✓")
    print("  get_land_use_name(999) = '类型999' (未定义) ✓")
    print("  获取土地利用类型名称测试通过 ✓")


def test_get_land_use_color():
    """测试获取土地利用类型颜色"""
    print("\n--- 测试: get_land_use_color ---")

    # 测试有林地颜色 (Value=21)
    color_21 = get_land_use_color(21)
    assert len(color_21) == 4
    assert color_21[3] == 255  # Alpha应为255
    print(f"  Value=21 (有林地) 颜色: RGB({color_21[0]},{color_21[1]},{color_21[2]}) ✓")

    # 测试未定义类型返回默认灰色
    color_unknown = get_land_use_color(9999)
    assert color_unknown == (128, 128, 128, 255)
    print(f"  Value=9999 (未定义) 颜色: RGB(128,128,128) (默认灰色) ✓")

    print("  获取土地利用类型颜色测试通过 ✓")


def test_build_land_use_legend_list():
    """测试土地利用类型图例列表构建"""
    print("\n--- 测试: build_land_use_legend_list ---")

    tif_path = resolve_path(LAND_USE_TIF_PATH)
    if os.path.exists(tif_path):
        land_use_list = build_land_use_legend_list(tif_path)
        if land_use_list:
            print(f"  成功构建 {len(land_use_list)} 个土地利用类型图例项 ✓")
            # 打印前5个图例项
            for item in land_use_list[:5]:
                value, color, name = item
                print(f"    Value={value}, Color=RGB({color[0]},{color[1]},{color[2]}), 名称={name}")
        else:
            print("  [信息] 图例列表为空，使用预定义分类")
    else:
        print(f"  [跳过] TIF文件不存在: {tif_path}")
        # 即使文件不存在，也应该能构建预定义的图例列表
        land_use_list = build_land_use_legend_list("")
        assert len(land_use_list) == len(LAND_USE_CLASSIFICATION)
        print(f"  使用预定义分类构建 {len(land_use_list)} 个图例项 ✓")

    print("  土地利用类型图例列表测试完成")
...
def test_wrap_text():
    ...
    # 测试短文本（不需要折行）
    result = _wrap_text("水田", 10)
    assert result == ["水田"]
    print("  '水田' (max=10) -> ['水田'] ✓")

    # 测试长文本（需要折行）
    result = _wrap_text("高覆盖度草地", 4)
    assert result == ["高覆盖度", "草地"]
    print("  '高覆盖度草地' (max=4) -> ['高覆盖度', '草地'] ✓")
...
def test_intensity_kml_parsing():
    """测试烈度圈KML解析相关函数"""
    print("\n--- 测试: 烈度圈KML解析 ---")

    # 测试从名称提取烈度值
    assert _extract_intensity_from_name("6度") == 6
    assert _extract_intensity_from_name("VI") == 6
    assert _extract_intensity_from_name("7") == 7
    assert _extract_intensity_from_name("VIII度") == 8
    assert _extract_intensity_from_name("invalid") is None

    print("  _extract_intensity_from_name('6度') = 6 ✓")
    print("  _extract_intensity_from_name('VI') = 6 ✓")
    print("  _extract_intensity_from_name('7') = 7 ✓")
    print("  _extract_intensity_from_name('VIII度') = 8 ✓")
    print("  _extract_intensity_from_name('invalid') = None ✓")

    # 测试KML坐标解析
    coords_text = "116.0,39.0,0 116.1,39.1,0 116.2,39.0,0"
    coords = _parse_kml_coords(coords_text)
    assert len(coords) == 3
    assert coords[0] == (116.0, 39.0)
    assert coords[1] == (116.1, 39.1)
    print(f"  KML坐标解析: 3个点 ✓")
...
def run_all_tests():
    """运行所有测试"""
    print("\n" + "=" * 60)
    print("运行 earthquake_land_use_map 全部测试")
    print("=" * 60)

    test_magnitude_config()
    test_calculate_extent()
    test_int_to_roman()
    test_land_use_classification()
    test_get_land_use_name()
    test_get_land_use_color()
    test_build_land_use_legend_list()
    test_boundary_styles()
    test_wrap_text()
    test_intensity_kml_parsing()
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].lower() == "test":
        ...
        try:
            ...
            out = sys.argv[4] if len(sys.argv) > 4 else f"earthquake_land_use_M{mag}_{lon}_{lat}.png"
            kml = sys.argv[5] if len(sys.argv) > 5 else None
            generate_earthquake_land_use_map(lon, lat, mag, out, kml)
        except ValueError as e:
            print(f"[错误] 参数格式错误: {e}")
            print("用法: python earthquake_land_use_map.py <经度> <纬度> <震级> [输出文件名] [kml路径]")
    else:
