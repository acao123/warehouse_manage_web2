...
- 统计输出图范围内滑坡和斜坡数量
- 图例位于主图右侧图例区，包含震中、地级市、省界、市界、县界、滑坡、斜坡
- 图例布局：基本图例项采用3行2列布局，滑坡和斜坡带"历史滑坡灾害"标题展示
"""
...
def create_print_layout(project, longitude, latitude, magnitude, extent, scale, map_height_mm, ordered_layers=None):
    ...
    # 添加图例到右侧图例区（包含比例尺）
    _add_legend(layout, map_item, map_height_mm, scale=scale, extent=extent, center_lat=latitude)
...
def _add_legend(layout, map_item, map_height_mm, scale=None, extent=None, center_lat=None):
    """
    添加图例（位于主图右侧图例区）
    - 上部：震中/地级市/省界/市界/县界（3行2列）
    - 中部：历史滑坡灾害标题 + 滑坡和斜坡图例项
    - 底部：比例尺
    ...
    参数:
        ...
        map_item: QgsLayoutItemMap, 地图项
        map_height_mm: float, 地图高度(毫米)
        scale: int或None, 比例尺分母
        extent: QgsRectangle或None, 地图范围
        center_lat: float或None, 地图中心纬度
    """
    ...
    # 基本图例项：3行2列（震中、地级市、省界、市界、县界）
    basic_legend_start_y = legend_y + 7.0

    col_count = 2
    row_count = 3  # ceil(5/2) = 3
    left_pad = 2.0
    right_pad = 2.0
    col_gap = 1.0
    row_height = 6.0
    ...
    # 历史滑坡灾害 标题（仿照高程图的标题样式）
    hazard_title_y = basic_legend_start_y + basic_legend_height + 2.0

    hazard_title_format = QgsTextFormat()
    hazard_title_cn_font = QFont("SimHei")
    hazard_title_cn_font.setPointSizeF(10.0)
    hazard_title_format.setFont(hazard_title_cn_font)
    hazard_title_format.setSize(10.0)
    ...
    hazard_legend_start_y = hazard_title_y + 5.5
    hazard_row_height = 8.0
    ...
    for idx, (display_name, fill_color, stroke_color, stroke_width, size) in enumerate(hazard_items):
        ...
        text_label.setTextFormat(item_format)
        text_label.attemptMove(QgsLayoutPoint(text_x, item_y + 0.5, QgsUnitTypes.LayoutMillimeters))
        text_label.attemptResize(QgsLayoutSize(text_width, hazard_row_height - 1.0,
                                               QgsUnitTypes.LayoutMillimeters))
        text_label.setHAlign(Qt.AlignLeft)
        text_label.setVAlign(Qt.AlignVCenter)
        text_label.setFrameEnabled(False)
...
if __name__ == "__main__":
    if len(sys.argv) >= 4:
        try:
            ...
            result, stats = generate_earthquake_landslide_slope_map(lon, lat, mag, out)
            print(f"\n{stats}")
        except ValueError as e:
            print(f"[错误] 参数格式错误: {e}")
            print("用法: python earthquake_landslide_slope_map.py <经度> <纬度> <震级> [输出文件名]")
    else:
