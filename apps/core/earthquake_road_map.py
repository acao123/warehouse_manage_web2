...
底图说明：
- 使用天地图WMTS服务作为底图
- 加载全国各级道路网SHP文件（高速、国道、省道、铁路、乡道、城市道路等）
- 图例展示各级道路的线型和颜色
"""
...
# 日志配置
# ============================================================
logger = logging.getLogger('report.core.earthquake_road_map')
...
# 道路数据文件路径
ROAD_DATA_BASE_PATH = (
    getattr(_django_settings, 'ROAD_DATA_PATH',
            _DEFAULT_BASE + '全国道路网shp数据/全国道路网/')
    if _DJANGO_AVAILABLE else _DEFAULT_BASE + '全国道路网shp数据/全国道路网/'
)
...
# === 道路样式配置 ===
# 格式: {道路类型: (颜色, 线宽mm, 是否虚线, 虚线间隔mm)}
ROAD_STYLE_CONFIG = {
    "高速": (QColor(255,158,23), 0.3, False, 0),
    "国道": (QColor(225,89,137), 0.25, False, 0),
    "省道": (QColor(255, 255, 0), 0.2, False, 0),
    "铁路": (QColor(0, 0, 0), 0.5, True, 0.4),
    "乡道": (QColor(213,227,240), 0.1, False, 0),

    "城市一级道路": (QColor(95,47,213), 0.25, False, 0),
    "城市二级道路": (QColor(74,141,152), 0.2, False, 0),
    "城市三级道路": (QColor(164,113,88), 0.17, False, 0),
    "城市四级道路": (QColor(152,227,124), 0.15, False, 0),
}
...
def load_road_layer(road_type):
    """
    加载指定类型的道路图层

    参数:
        road_type: str, 道路类型（如"高速"、"国道"等）

    返回:
        QgsVectorLayer或None, 道路图层
    """
    if road_type not in ROAD_SHP_FILES:
        print(f"[错误] 未知的道路类型: {road_type}")
        return None

    shp_filename = ROAD_SHP_FILES[road_type]
    shp_path = resolve_path(ROAD_DATA_BASE_PATH + shp_filename)

    if not os.path.exists(shp_path):
        print(f"[警告] 道路文件不存在: {shp_path}")
        return None

    layer = QgsVectorLayer(shp_path, road_type, "ogr")

    if not layer.isValid():
        print(f"[错误] 无法加载道路图层: {shp_path}")
        return None

    # 设置道路样式
    _style_road_layer(layer, road_type)

    print(f"[信息] 成功加载道路图层 '{road_type}': {shp_path}")
    return layer


def _style_road_layer(layer, road_type):
    """
    设置道路图层样式

    参数:
        layer: QgsVectorLayer, 道路图层
        road_type: str, 道路类型
    """
    ...
    line_sl.setWidth(line_width)
    line_sl.setWidthUnit(QgsUnitTypes.RenderMillimeters)

    if is_dashed and dash_gap > 0:
        line_sl.setPenStyle(Qt.CustomDashLine)
        # 虚线模式: [线段长度, 间隔长度]
        dash_pattern = [4.0, dash_gap / line_width]
        if hasattr(line_sl, 'setCustomDashVector'):
    ...
    print(
        f"[信息] 道路图层 '{road_type}' 样式设置完成 - 颜色: RGB({color.red()},{color.green()},{color.blue()}), 线宽: {line_width}mm")
...
def create_road_legend_layer(road_type):
    """
    创建道路图例用的线图层
    ...
    layer = QgsVectorLayer("LineString?crs=EPSG:4326", road_type, "memory")
    provider = layer.dataProvider()
    provider.addAttributes([QgsField("name", QVariant.String)])
    layer.updateFields()
...
def _add_legend(layout, map_item, project, map_height_mm, output_height_mm,
    ...
    # 道路图例文本格式
    road_format = QgsTextFormat()
    road_format.setFont(QFont("SimSun", LEGEND_ROAD_FONT_SIZE_PT))
    road_format.setSize(LEGEND_ROAD_FONT_SIZE_PT)
    road_format.setSizeUnit(QgsUnitTypes.RenderPoints)
    road_format.setColor(QColor(0, 0, 0))
    ...
    current_y = road_item_start_y
    for road_type in road_legend_order:
        if road_type not in ROAD_STYLE_CONFIG:
            continue
        ...
        color, line_width, is_dashed, dash_gap = ROAD_STYLE_CONFIG[road_type]
        icon_center_y = current_y + road_item_height / 2.0

        # 绘制道路图标
        if is_dashed:
            _draw_dash_line_icon(layout, legend_x + left_pad, icon_center_y,
                                 road_icon_width, color, line_width, dash_gap)
        else:
            _draw_line_icon(layout, legend_x + left_pad, icon_center_y,
                            road_icon_width, color, line_width, solid=True)

        # 添加道路名称标签
        text_x = legend_x + left_pad + road_icon_width + road_icon_text_gap
        text_width = legend_width - left_pad - road_icon_width - road_icon_text_gap - right_pad

        text_label = QgsLayoutItemLabel(layout)
        text_label.setText(road_type)
        text_label.setTextFormat(road_format)
        text_label.attemptMove(QgsLayoutPoint(text_x, current_y, QgsUnitTypes.LayoutMillimeters))
        text_label.attemptResize(QgsLayoutSize(text_width, road_item_height, QgsUnitTypes.LayoutMillimeters))
        text_label.setHAlign(Qt.AlignLeft)
        text_label.setVAlign(Qt.AlignVCenter)
        text_label.setFrameEnabled(False)
        text_label.setBackgroundEnabled(False)
...
def generate_earthquake_road_map(longitude, latitude, magnitude,
    ...
    """
    logger.info('开始生成道路交通图: lon=%.4f lat=%.4f M=%.1f output=%s',
...
def _generate_earthquake_road_map_impl(longitude, latitude, magnitude, output_path,
                                       basemap_path=None, annotation_path=None):
    ...
    try:
        ...
        # 加载道路图层（按优先级从低到高加载，高优先级图层在上层显示）
        road_load_order = [
            "城市四级道路", "城市三级道路", "城市二级道路", "城市一级道路",
            "乡道", "省道", "国道", "铁路", "高速"
        ]
        ...
        # 创建道路图例图层
        for road_type in ROAD_STYLE_CONFIG.keys():
            road_legend_layer = create_road_legend_layer(road_type)
            if road_legend_layer:
                project.addMapLayer(road_legend_layer)
...
def test_road_style_config():
    """测试道路样式配置"""
    print("\n--- 测试: 道路样式配置 ---")

    # 测试高速配置
    assert "高速" in ROAD_STYLE_CONFIG
    color, width, is_dashed, gap = ROAD_STYLE_CONFIG["高速"]
    assert color.red() == 255 and color.green() == 0 and color.blue() == 0
    assert width == 0.6
    assert is_dashed is False
    print(f"  高速: RGB({color.red()},{color.green()},{color.blue()}), 线宽{width}mm, 实线 ✓")

    # 测试铁路配置
    assert "铁路" in ROAD_STYLE_CONFIG
    color, width, is_dashed, gap = ROAD_STYLE_CONFIG["铁路"]
    assert color.red() == 0 and color.green() == 0 and color.blue() == 0
    assert is_dashed is True
    print(f"  铁路: RGB({color.red()},{color.green()},{color.blue()}), 线宽{width}mm, 虚线间隔{gap}mm ✓")

    # 测试国道配置
    assert "国道" in ROAD_STYLE_CONFIG
    color, width, is_dashed, gap = ROAD_STYLE_CONFIG["国道"]
    assert color.red() == 255 and color.green() == 165 and color.blue() == 0
    print(f"  国道: RGB({color.red()},{color.green()},{color.blue()}), 线宽{width}mm, 实线 ✓")
...
def test_tianditu_config():
    ...
    # 验证URL格式
    assert "tianditu.gov.cn" in TIANDITU_VEC_URL
    assert TIANDITU_TK in TIANDITU_VEC_URL
    print(f"  天地图矢量底图URL包含正确的域名和Token ✓")

    print("  天地图配置测试通过 ✓")


def test_road_shp_paths():
    """测试道路SHP文件路径配置"""
    print("\n--- 测试: 道路SHP文件路径配置 ---")

    expected_roads = ["高速", "国道", "省道", "铁路", "乡道",
                      "城市一级道路", "城市二级道路", "城市三级道路", "城市四级道路"]

    for road in expected_roads:
        assert road in ROAD_SHP_FILES, f"缺少道路类型: {road}"
        shp_file = ROAD_SHP_FILES[road]
        assert shp_file.endswith(".shp"), f"{road} 文件扩展名不正确"
        print(f"  {road}: {shp_file} ✓")

    print("  道路SHP文件路径配置测试通过 ✓")
...
def run_all_tests():
    """运行所有测试"""
    print("\n" + "=" * 60)
    print("运行 earthquake_road_map 全部测试")
    print("=" * 60)

    test_magnitude_config()
    test_calculate_extent()
    test_road_style_config()
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].lower() == "test":
        ...
        try:
            ...
            out = sys.argv[4] if len(sys.argv) > 4 else f"earthquake_road_M{mag}_{lon}_{lat}.png"
            generate_earthquake_road_map(lon, lat, mag, out)
        except ValueError as e:
            print(f"[错误] 参数格式错误: {e}")
            print("用法: python earthquake_road_map.py <经度> <纬度> <震级> [输出文件名]")
    else:
