...
处理流程：
    ...
    4. 使用QGIS Processing框架IDW/TIN插值算法生成栅格
...
class KmlToIaConverter:
    """
    KML文件转Ia(阿里亚斯强度)栅格文件转换器（QGIS 3.40.15 Processing 版）

    将地震局提供的KML格式PGA等值线文件，经过解析、坐标转换、
    插值计算后，输出Ia.tif栅格文件（可选输出PGA.tif）。

    本版本使用 QGIS 3.40.15 Processing 框架的 IDW / TIN 插值算法，
    并针对 32G 内存运行环境进行了全面的内存优化。
    ...
    def _transform_coords_batch(
        ...
        参数:
            ...
            chunk_size (int): 每批处理的点数，默��10000
    ...
    def _build_qgs_vector_layer(
        ...
        # 使用字符串类型名称替代 QVariant.Double，兼容新版 QGIS
        ...
        # 将图层添加到项目中（Processing 算法需要从项目中读取图层）
    ...
    def _run_qgis_interpolation(
        ...
        """
        使用 QGIS Processing 框架的 IDW/TIN 插值算法生成栅格文件。

        根据 self.interp_method 选择 IDW 或 TIN 插值算法，
        通过 processing.run() 执行插值并直接输出 GeoTIFF 文件。
        ...
        # 构建 QGIS 内存矢量图层
        layer = self._build_qgs_vector_layer(x_utm, y_utm, values, field_name='value')

        # 获取图层 ID
        layer_id = layer.id()

        # 构建范围字符串（格式：xmin,xmax,ymin,ymax [CRS]）
        extent_str = f"{self._x_min},{self._x_max},{self._y_min},{self._y_max} [EPSG:{self._utm_epsg}]"

        method = self.interp_method.lower()

        try:
            if method == 'idw':
                # 使用 qgis:idwinterpolation 算法
                # INTERPOLATION_DATA 格式: "layer_id::~::use_z_coord::~::attr_index::~::source_type"
                # use_z_coord: 0=使用属性值, 1=使用Z坐标
                # attr_index: 属性字段索引（0 表示第一个字段 'value'）
                # source_type: 0=点, 1=结构线, 2=断裂线
                interp_data = f"{layer_id}::~::0::~::0::~::0"

                params = {
                    'INTERPOLATION_DATA': interp_data,
                    'DISTANCE_COEFFICIENT': self.idw_power,
                    'EXTENT': extent_str,
                    'PIXEL_SIZE': self.resolution,
                    'OUTPUT': output_tif_path,
                }

                print(f"  执行 IDW 插值...")
                print(f"    距离系数: {self.idw_power}")
                print(f"    像素大小: {self.resolution}m")

                result = processing.run('qgis:idwinterpolation', params)

            elif method == 'tin':
                # 使用 qgis:tininterpolation 算法
                interp_data = f"{layer_id}::~::0::~::0::~::0"

                params = {
                    'INTERPOLATION_DATA': interp_data,
                    'METHOD': self.tin_method,  # 0=Linear, 1=Clough-Tocher
                    'EXTENT': extent_str,
                    'PIXEL_SIZE': self.resolution,
                    'OUTPUT': output_tif_path,
                }

                method_name = "Linear" if self.tin_method == 0 else "Clough-Tocher"
                print(f"  执行 TIN 插值 ({method_name})...")
                print(f"    像素大小: {self.resolution}m")
            ...
            print(f"  插值完成，输出文件: {result.get('OUTPUT', output_tif_path)}")
    ...
    def _interpolate_to_file(
        ...
        """
        统一插值入口：调用 QGIS Processing 框架执行插值并输出 GeoTIFF。
    ...
    def run(self) -> bool:
        ...
        流程:
            ...
            4. 构建输出栅格网格
            5. （可选）PGA等值线矢量栅格��并输出PGA.tif
            6. 使用 QGIS Processing IDW/TIN 插值计算并输出Ia.tif
            7. 打印耗时统计
        ...
        """
        print("=" * 60)
        print("KML → Ia 栅格处理程序（QGIS 3.40.15 Processing）")
        ...
        # 8. Ia插值（QGIS Processing IDW/TIN）
        print("\n" + "-" * 40)
        print(f"步骤: Ia插值计算（QGIS Processing {self.interp_method.upper()}）")
        print("-" * 40)
